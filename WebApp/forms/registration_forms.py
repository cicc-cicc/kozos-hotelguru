...
class RegistrationForm(FlaskForm):
    username = StringField(
        "Felhasználónév", validators=[DataRequired(), Length(min=4, max=20)]
    )
    email = StringField(
        "E-mail cím",
        validators=[
    ...
    password = PasswordField(
        "Jelszó", validators=[DataRequired(), Length(min=8), validate_password_strength]
    )
    confirm_password = PasswordField(
        "Jelszó ismét",
        validators=[
            DataRequired(),
            EqualTo("password", message="A jelszavaknak egyezniük kell!"),
        ],
    )
    ...
    address = StringField(
        "Lakcím", validators=[DataRequired(message="A számlázáshoz kötelező megadni!")]
    )
