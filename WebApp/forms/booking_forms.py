...
class RoomSearchForm(FlaskForm):
    ...
    guests = IntegerField(
        "Vendégek száma",
        validators=[
            DataRequired(message="Kérjük, adja meg a vendégek számát!"),
            NumberRange(min=1, message="Legalább egy vendégnek kell lennie!"),
        ],
    )
        ...
        if self.arrival_date.data and self.departure_date.data:
            if self.departure_date.data <= self.arrival_date.data:
                self.departure_date.errors.append(
                    "A távozás dátuma későbbi kell legyen, mint az érkezés dátuma."
                )
                return False
...
class BookingCancelForm(FlaskForm):
    confirm = BooleanField(
        "Biztosan lemondod a foglalást?", validators=[DataRequired()]
    )
...
class BookingEditForm(FlaskForm):
    arrival_date = DateField(
        "Érkezés napja", format="%Y-%m-%d", validators=[DataRequired()]
    )
    departure_date = DateField(
        "Távozás napja", format="%Y-%m-%d", validators=[DataRequired()]
    )
    guests = IntegerField(
        "Vendégek száma", validators=[DataRequired(), NumberRange(min=1)]
    )
