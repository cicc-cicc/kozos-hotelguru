...
def create_app():
    ...
    with app.app_context():
        ...
        )  # Minden auth route kap egy /auth előtagot (pl. /auth/login)

        from .routes.guests_routes import guest_bp

        app.register_blueprint(
            guest_bp
        )  # Vendég útvonalak a gyökerére kerülnek (pl. /search-results)
        ...
        app.register_blueprint(
            reception_bp, url_prefix="/reception"
        )  # Recepciós útvonalak /reception előtaggal (pl. /reception/dashboard)

        from .routes.admin_routes import admin_bp

        app.register_blueprint(
            admin_bp, url_prefix="/admin"
        )  # Admin útvonalak /admin előtaggal (pl. /admin/dashboard)
