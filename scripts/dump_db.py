...
with app.app_context():
    ...
    data['booking_services'] = [
        {'id': bs.id, 'booking_id': bs.booking_id, 'service_id': bs.service_id, 'quantity': bs.quantity}
        for bs in BookingService.query.all()
    ]

    data['invoices'] = [
        {'id': inv.id, 'booking_id': inv.booking_id, 'total_amount': inv.total_amount, 'paid': inv.paid, 'created_at': serialize(inv.created_at)}
