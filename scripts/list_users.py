...
with app.app_context():
    ...
    for u in users:
        print(f"- id={u.id} username={u.username} email={u.email} role={u.role.name} created={u.created_at}")
    ...
    if os.path.exists(data_path):
        ...
        missing = [name for name in json_users if not any(u.username == name for u in users)]
